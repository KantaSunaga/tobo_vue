  ...
  <div>
    ...
    <div class="task-list">
      <label class="task-list__item"v-for="todo in todos" v-bind:class="{ 'task-list__item--checked': todo.done }">
        <i class="fas fa-check-circle" v-if="todo.done" @click="todo.done = !todo.done"></i>
        <input type="checkbox" v-else v-model="todo.done">
...
export default {
  ...
  data () {
    return {
      ...
      todos : [
        {text : 'vue-router', dane: false, editing: false},
